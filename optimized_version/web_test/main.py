...
from ECA_MobileNetV3Small import mobilenet_v3_small
...
def transform_image(image_bytes):
    ...
    return (
        my_transforms(image).unsqueeze(0).to(device)
    )  # 新增一个batch维度并转移到device设备
...
def get_prediction(image_bytes):
    try:
        ...
        outputs = torch.softmax(
            model.forward(tensor).squeeze(), dim=0
        )  # 将预测值转化为概率分布
        prediction = (
            outputs.detach().cpu().numpy()
        )  # detach方法去除梯度信息，转移到CPU并转化为numpy格式。
        template = "class:{:<15} probability:{:.3f}"
        index_pre = [
            (class_indict[str(index)], float(p)) for index, p in enumerate(prediction)
        ]
        # sort probability
