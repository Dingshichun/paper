"""
生成 EfficientNet 模型的代码，包括 EfficientNet 的 B0 到 B7 版本。
"""
...
class ConvBNActivation(nn.Sequential):
    def __init__(
        ...
        norm_layer: Optional[
            Callable[..., nn.Module]
        ] = None,  # MBConv中的BN结构，即批归一化，默认为None
...
class InvertedResidualConfig:
    ...
    ):  # 网络宽度缩放的倍率因子，宽度指的就是卷积核的个数。b0到b7倍率不同。
        ...
        layers.update(
            {
                "dwconv": ConvBNActivation(
                    cnf.expanded_c,  # 经过DW卷积时，通道数不改变，所以输入通道数和输出通道数都是expanded_c
                    cnf.expanded_c,
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        if norm_layer is None:
            norm_layer = partial(
                nn.BatchNorm2d, eps=1e-3, momentum=0.1
            )  # 构建BN层需要设置的超参数
        ...
        for stage, args in enumerate(
            default_cnf
        ):  # 遍历每个stage，enumerate方法得到数据的同时返回数据的索引。
            cnf = copy.copy(args)
