"""
ECA_MobileNetV3 实现的代码。
主要是将原来 MobileNetV3 结构中的 SE 模块替换为 ECA 模块。
"""
...
class InvertedResidual(nn.Module):
    def __init__(
        ...
        activation_layer = (
            nn.Hardswish if cnf.use_hs else nn.ReLU
        )  # 判断使用哪种激活函数
        ...
        # 如果使用SE模块，则创建SE到layer中，
        # 主要将这里更改为ECA结构，即要创建SE的全部改为创建ECA。
        if cnf.use_se:
            layers.append(ECA(cnf.expanded_c))
