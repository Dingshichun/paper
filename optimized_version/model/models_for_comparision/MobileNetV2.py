...
def main(args):
    ...
    data_transform = {
        "train": transforms.Compose(
            [
                # 将原图大小变为299×299,传入元组，不能只传一个数字，
                # 传一个数字时默认是调整短边到传入的大小，长边则按同比例变化。
    ...
    model.classifier = torch.nn.Sequential(
        ...
        torch.nn.Linear(in_features=1280, out_features=args.num_class, bias=True),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img-size", type=int, default=299, help="the size of input image"
    )  # 输入图像的尺寸
    ...
    parser.add_argument(
        "--RunData-save-path",
        type=str,
        default="./Data/MobileNetV2/RunData",
    ...
    parser.add_argument(
        "--model-save-path",
        type=str,
        default="./Data/MobileNetV2/model/MobileNetV2-",
