...
def main(args):
    ...
    data_transform = {
        "train": transforms.Compose(
            [
                # AlexNet 输入的图像大小为227×227,只传入一个参数时默认该参数为图像短边，长边会同比例放大。
                # 所以需要传入一个元组（227×227），将其长短边都进行指定才能得到想要的效果。
                transforms.Resize((args.img_size, args.img_size)),
                transforms.RandomRotation(90),  # 随机旋转 0~90 度
    ...
    optimizer = optim.SGD(
        pg, lr=args.lr, momentum=0.9, weight_decay=1e-4
    )  # weight_decay是权重衰减，防止过拟合。
    for name, para in model.named_parameters():
    ...
    for epoch in range(args.epochs):
        ...
        # optimizer.zero_grad() # 梯度清零，这里不用清零，
        # 因为train_one_epoch函数中已经包含清零和更新参数的功能。
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--img-size", type=int, default=227, help="the size of input image"
    )  # 输入图像的尺寸
    ...
    parser.add_argument(
        "--model-save-path",
        type=str,
        default="./Data/AlexNet/model/AlexNet-",
