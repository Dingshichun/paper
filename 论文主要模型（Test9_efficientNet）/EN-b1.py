...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Test9_efficientNet/RunData/EN-b1/run')
    # tensorboard文件保存的路径
    if os.path.exists("D:/FoodScience/Test9_efficientNet/RunData/EN-b1/run") is False:
        os.makedirs("D:/FoodScience/Test9_efficientNet/RunData/EN-b1/run")
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Test9_efficientNet/RunData/EN-b1/model/EN-b1-{}-state_dict.pth".format(
                               max_acc_index))
                # 保存证准确率最高的模型参数和架构
                torch.save(model,
                           "D:/FoodScience/Test9_efficientNet/RunData/EN-b1/model/EN-b1-{}-all.pth".format(
                               max_acc_index))
                break
            else:
                continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./efficientnetb1.pth',
