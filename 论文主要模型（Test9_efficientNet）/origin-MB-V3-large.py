...
from origin_model_v3 import mobilenet_v3_large
...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Data/origin-MB-V3-large/RunData')
    # tensorboard文件保存的路径
    if os.path.exists("D:/FoodScience/Data/origin-MB-V3-large/RunData") is False:
        os.makedirs("D:/FoodScience/Data/origin-MB-V3-large/RunData")
    ...
    pretrained_dict={k: v for k, v in MB_V3_large_pretrained_dict.items() if k in model_dict and (v.shape == model_dict[k].shape)}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict, strict=False)
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Data/origin-MB-V3-large/ModelSave/origin-MB-V3-large-{}-state_dict.pth".format(
                               max_acc_index))
                # 保存证准确率最高的模型参数和架构
                torch.save(model,
                           "D:/FoodScience/Data/origin-MB-V3-large/ModelSave/origin-MB-V3-large-{}-all.pth".format(
                               max_acc_index))
                break
            else:
                continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./mobilenet_v3_large.pth',
