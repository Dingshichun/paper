...
from origin_model_v3 import mobilenet_v3_small
...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Data/origin-MB-V3/RunData')
    # tensorboard文件保存的路径
    if os.path.exists("D:/FoodScience/Data/origin-MB-V3/RunData") is False:
        os.makedirs("D:/FoodScience/Data/origin-MB-V3/RunData")
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Data/origin-MB-V3/ModelSave/origin-MB-v3-{}-state_dict.pth".format(
                               max_acc_index))
                # 保存证准确率最高的模型参数和架构
                torch.save(model,
                           "D:/FoodScience/Data/origin-MB-V3/ModelSave/origin-MB-v3-{}-all.pth".format(
                               max_acc_index))
                break
            else:
                continue
