...
class ConvBNActivation(nn.Sequential):
    def __init__(self,
        ...
        if activation_layer is None:  # 如果激活函数为空，将其设置为Hardswish
            activation_layer = nn.Hardswish  # alias Swish  (torch>=1.7)
...
# ECA模块，效果优于SE模块，将MBCov中的SE全部替换为ECA
...
class InvertedResidual(nn.Module):
    def __init__(self,
        ...
        layers = OrderedDict()
        activation_layer = nn.Hardswish  # alias Swish
        ...
        #                                            cnf.expanded_c)})
        if cnf.use_se:
            layers.update({"ECA": ECA_layer(cnf.input_c)})
...
class EfficientNet(nn.Module):
    def __init__(self,
        ...
        # 增加通道注意力和空间注意力，后面的前向传播也要有。
        # self.ca1 = ChannelAttention(in_planes=32)  # 要加入参数，输入通道数
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # 加入CBAM注意力，只能加在block前，不然无法使用预训练参数
        # x = nn.Hardswish(self.BN(self.conv_stem(x)))
