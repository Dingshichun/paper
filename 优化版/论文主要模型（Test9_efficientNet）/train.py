...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Test9_efficientNet/RunData/CBAM-EN-b0/run')
    # tensorboard文件保存的路径
    if os.path.exists("D:/FoodScience/Test9_efficientNet/RunData/CBAM-EN-b0/run") is False:
        os.makedirs("D:/FoodScience/Test9_efficientNet/RunData/CBAM-EN-b0/run")
    ...
    EN_b0_pretrained_dict=torch.load(args.weights,map_location=device)
    model_dict=model.state_dict()
    pretrained_dict={k: v for k, v in EN_b0_pretrained_dict.items() if k in model_dict and (v.shape == model_dict[k].shape)}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict, strict=False)
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Test9_efficientNet/RunData/CBAM-EN-b0/model/CBAM-EN-b0-{}-state_dict.pth".format(
                               max_acc_index))
                # 保存证准确率最高的模型参数和架构
                torch.save(model,
                           "D:/FoodScience/Test9_efficientNet/RunData/CBAM-EN-b0/model/CBAM-EN-b0-{}-all.pth".format(
                               max_acc_index))
                break
            else:
                continue
