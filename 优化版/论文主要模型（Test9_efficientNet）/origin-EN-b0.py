...
from EN_b0_model import efficientnet_b0 as create_model
...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Test9_efficientNet/RunData/origin-ENb0/run')
    # tensorboard文件保存的路径
    if os.path.exists("D:/FoodScience/Test9_efficientNet/RunData/origin-ENb0/run") is False:
        os.makedirs("D:/FoodScience/Test9_efficientNet/RunData/origin-ENb0/run")
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Test9_efficientNet/RunData/origin-ENb0/model/origin-ENb0-{}-state_dict.pth".format(
                               max_acc_index))
                # 保存证准确率最高的模型参数和架构
                torch.save(model,
                           "D:/FoodScience/Test9_efficientNet/RunData/origin-ENb0/model/origin-ENb0-{}-all.pth".format(
                               max_acc_index))
                break
            else:
                continue
