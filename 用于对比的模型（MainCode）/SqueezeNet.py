...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Data/SqueezeNet/RunData') # 括号内设置数据可视化路径,默认在当前文件路径创建runs文件夹
    # 该路径用来保存训练的模型
    if os.path.exists("D:/FoodScience/Data/SqueezeNet/ModelSave") is False:
        os.makedirs("D:/FoodScience/Data/SqueezeNet/ModelSave")
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Data/SqueezeNet/ModelSave/SqueezeNet-{}-state_dict.pth".format(max_acc_index))
                # 保存验证集准确率最高的模型参数和架构
                torch.save(model, "D:/FoodScience/Data/SqueezeNet/ModelSave/SqueezeNet-{}-all.pth".format(max_acc_index))
                break
            else:
                continue
