...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Data/EfficientNet-b2/RunData') # 括号内设置数据可视化路径,默认在当前文件路径创建runs文件夹
    # 该路径用来保存训练的模型
    if os.path.exists("D:/FoodScience/Data/EfficientNet-b2/ModelSave") is False:
        os.makedirs("D:/FoodScience/Data/EfficientNet-b2/ModelSave")
    ...
    data_transform = {
        "train": transforms.Compose([
            transforms.Resize((260, 260)),  # EffficientNet_B0输入224×224，将原图大小变为224×224
            transforms.RandomHorizontalFlip(),  # 随机水平翻转
            transforms.ToTensor(),  # 张量
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # 标准化
        ]),
        "val": transforms.Compose([
            transforms.Resize((260, 260)),  # 将原图大小变为224×224
            transforms.RandomHorizontalFlip(),  # 随机水平翻转
            transforms.ToTensor(),  # 张量
            transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # 标准化
        ])
        ...
        if epoch > 11:
            if val_acc_list[epoch]<max_acc and epoch - max_acc_index >=11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Data/EfficientNet-b2/ModelSave/EfficientNet_B2-{}-state_dict.pth".format(
                               max_acc_index))
                # 保存证准确率最高的模型参数和架构
                torch.save(model,
                           "D:/FoodScience/Data/EfficientNet-b2/ModelSave/EfficientNet_B2-{}-all.pth".format(
                               max_acc_index))
                break
            else:
                continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size',type=int,default=16) # batchsize太小，考虑增大。32或者40感觉可行。
    parser.add_argument('--lr',type=float,default=0.01)
    # parser.add_argument('--lrf',type=float,default=0.01)
    # 数据路径
    parser.add_argument('--data_path',type=str,
                        default="E:/HongShu/NewHongShu/HongShu/TrainVal")
