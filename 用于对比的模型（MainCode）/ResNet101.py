...
def main(args):
    ...
    tb_writer = SummaryWriter('D:/FoodScience/Data/ResNet101/RunData') # 括号内设置数据可视化路径,默认在当前文件路径创建runs文件夹
    # 该路径用来保存训练的模型
    if os.path.exists("D:/FoodScience/Data/ResNet101/ModelSave") is False:
        os.makedirs("D:/FoodScience/Data/ResNet101/ModelSave")
    ...
    # 更改最后的输出种类数以满足自己的数据
    model.fc = torch.nn.Linear(in_features=feature, out_features=4, bias=True)
        ...
        if epoch > 11:
            if val_acc_list[epoch] < max_acc and epoch - max_acc_index >= 11:
                ...
                torch.save(model.state_dict(),
                           "D:/FoodScience/Data/ResNet101/ModelSave/ResNet101-{}-state_dict.pth".format(max_acc_index))
                # 保存验证集准确率最高的模型参数和架构
                torch.save(model, "D:/FoodScience/Data/ResNet101/ModelSave/ResNet101-{}-all.pth".format(max_acc_index))
                break
            else:
                continue
